...
"""
    ...
    def _normalize_reasoning_output(
        ...
        # --------------------------------------------------------------
        # Reject root causes that are not supported by CURRENT evidence.
        #
        # The knowledge-retrieval agent is intentionally excluded here.
        # Historical incidents can suggest hypotheses, but they cannot
        # establish the root cause of the current incident.
        # --------------------------------------------------------------

        if root_cause and not self._has_current_evidence_support(
            root_cause,
            agent_outputs,
        ):
            root_cause = None
            ...
            normalized["explanation"] = (
                "The reasoning model proposed a root cause that is not "
                "supported by the current specialized-agent evidence. "
                "Historical evidence alone is insufficient to establish "
    ...
    # ------------------------------------------------------------------
    # CURRENT-EVIDENCE SUPPORT CHECK
    # ------------------------------------------------------------------

    @staticmethod
    def _has_current_evidence_support(
        root_cause: Any,
        agent_outputs: dict[str, Any],
    ) -> bool:
        """
        Check whether an LLM-proposed root cause has meaningful support
        in the CURRENT specialized-agent findings.

        Historical knowledge-retrieval output is intentionally excluded.
        This prevents the reasoning model from promoting an unrelated
        historical incident into the current incident's root cause.

        The check is deliberately conservative: if no meaningful
        domain-specific term from the proposed cause occurs in the current
        specialized-agent evidence, the cause is treated as unsupported and
        ...
        current_outputs = {
            "log_analysis_agent": agent_outputs.get(
                "log_analysis_agent",
                {},
            ),
            "metrics_analysis_agent": agent_outputs.get(
                "metrics_analysis_agent",
                {},
            ),
            "source_code_analysis_agent": agent_outputs.get(
                "source_code_analysis_agent",
                {},
            ),
            "trace_analysis_agent": agent_outputs.get(
                "trace_analysis_agent",
                {},
            ),
        }
        ...
        # Words this short/generic are not useful for establishing that a
        # proposed cause is actually represented in current evidence.
        stop_words = {
            "about",
            "after",
            "again",
        ...
        words = {
            word.strip(".,:;!?()[]{}\"'")
            for word in root_cause_text.replace("-", " ").split()
        }
        ...
        return any(word in evidence_text for word in meaningful_words)
